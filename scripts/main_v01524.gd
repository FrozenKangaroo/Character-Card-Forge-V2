extends "res://scripts/main_v01523.gd"

const WORKSPACE_V01524 = preload("res://scripts/ui/workspace_v01524.gd")
const BUILD_DISPLAY_VERSION_V01524 := "0.15.24"


func _ready() -> void:
	super._ready()
	# Explicitly re-run the versioned Workspace installer after the inherited
	# startup chain. This makes the live Workspace/service composition a runtime
	# invariant instead of relying on older inherited _ready() dispatch details.
...
func _install_workspace_v0133() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V01524:
		return
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V01524.new()
...
func _update_build_version_label() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V01524
