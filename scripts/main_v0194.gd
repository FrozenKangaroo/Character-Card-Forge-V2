extends "res://scripts/main_v0193_hotfix1.gd"

const WORKSPACE_V0194 = preload("res://scripts/ui/workspace_v0194.gd")
const BUILD_DISPLAY_VERSION_V0194 := "0.19.4"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0194()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0194:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0194.new()
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0194
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0194() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0194
			node.tooltip_text = (
				"v0.19.4 adds local PDF text-layer extraction and review-first HTTPS "
				+ "reference ingestion with managed copies, provenance and explicit refresh."
			)
			return
