extends "res://scripts/main_v01536.gd"

const WORKSPACE_V01536_HOTFIX1 = preload(
	"res://scripts/ui/workspace_v01536_hotfix1.gd"
)
const BUILD_DISPLAY_VERSION_V01536_HOTFIX1 := "0.15.36-hotfix1"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v01536_hotfix1()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V01536_HOTFIX1:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V01536_HOTFIX1.new()
...
func _create_new_character() -> void:
	...
	_global_status.text = "New character project created using your configured default template"
	_refresh_home_and_library()


func _update_build_version_label_v01536_hotfix1() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V01536_HOTFIX1
			node.tooltip_text = (
