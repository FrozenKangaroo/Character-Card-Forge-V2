extends "res://scripts/main_v0186.gd"

const WORKSPACE_V0190 = preload("res://scripts/ui/workspace_v0190.gd")
const BUILD_DISPLAY_VERSION_V0190 := "0.19.0"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0190()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0190:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0190.new()
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0190
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0190() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0190
			node.tooltip_text = (
				"v0.19.0 adds reusable scenario setups, a weighted greeting manager, "
				+ "ensemble authoring, recoverable split-set generation, shared-world "
				+ "dependencies, lineage views and explicit private metadata mappings."
			)
			return
