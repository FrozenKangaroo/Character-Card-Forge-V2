extends "res://scripts/main_v0171.gd"

const WORKSPACE_V0172 = preload("res://scripts/ui/workspace_v0172.gd")
const BUILD_DISPLAY_VERSION_V0172 := "0.17.2"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0172()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0172:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0172.new()
...
func _update_build_version_label_v0172() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0172
			node.tooltip_text = (
				"Development build version. v0.17.2 adds optional Front Porch 2.5 "
				+ "character extensions with manual editing, selective AI proposals, "
				+ "review-first application and lossless unknown-field round-tripping."
			)
			return
