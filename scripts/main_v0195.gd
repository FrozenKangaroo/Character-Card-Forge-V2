extends "res://scripts/main_v0194.gd"

const WORKSPACE_V0195 = preload("res://scripts/ui/workspace_v0195.gd")
const SETTINGS_VIEW_V0195 = preload("res://scripts/ui/settings_view_v0195.gd")
const BUILD_DISPLAY_VERSION_V0195 := "0.19.5"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0195()
...
func _install_settings_view_v01528() -> void:
	...
	if previous_settings != null and previous_settings.get_script() == SETTINGS_VIEW_V0195:
	...
	var upgraded: CCFSettingsView = SETTINGS_VIEW_V0195.new()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0195:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0195.new()
	upgraded.visible = should_be_visible
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0195
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0195() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0195
			node.tooltip_text = (
				"v0.19.5 adds optional Fast / Suggestion and Deep Review Text routing, "
				+ "plus one-hop technical fallback with visible producer provenance."
			)
			return
