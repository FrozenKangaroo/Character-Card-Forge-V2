extends "res://scripts/main_v0162.gd"

const IMAGE_WINDOW_V0163 = preload(
	"res://scripts/ui/image_generation_window_v0163.gd"
)
const BUILD_DISPLAY_VERSION_V0163 := "0.16.3"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0163()


func _install_image_window_v01529() -> void:
	var previous := _image_generation_window
	if previous != null and previous.get_script() == IMAGE_WINDOW_V0163:
	...
	if previous != null:
		...
		previous.queue_free()
	var upgraded := IMAGE_WINDOW_V0163.new()
...
func _update_build_version_label_v0163() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0163
			node.tooltip_text = (
				"Development build version. v0.16.3 reorganises Image Studio into Prompt & Results, Creative, and Advanced tabs while preserving the v0.16.2 composer and v0.16.1 capability model."
			)
			return
