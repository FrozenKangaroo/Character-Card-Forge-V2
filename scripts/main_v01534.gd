extends "res://scripts/main_v01533_hotfix3.gd"

const WORKSPACE_V01534 = preload(
	"res://scripts/ui/workspace_v01534.gd"
)
const BUILD_DISPLAY_VERSION_V01534 := "0.15.34"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v01534()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V01534:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V01534.new()
...
func _update_build_version_label_v01534() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V01534
			node.tooltip_text = (
