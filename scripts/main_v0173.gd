extends "res://scripts/main_v0172.gd"

const WORKSPACE_V0173 = preload("res://scripts/ui/workspace_v0173.gd")
const BUILD_DISPLAY_VERSION_V0173 := "0.17.3"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0173()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0173:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0173.new()
...
func _update_build_version_label_v0173() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0173
			node.tooltip_text = (
				"Development build version. v0.17.3 adds authenticated, user-initiated "
				+ "Front Porch direct install through its supported character import API, "
				+ "explicit collision choices and portable JSON fallback."
			)
			return
