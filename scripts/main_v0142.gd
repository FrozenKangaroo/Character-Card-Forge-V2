extends "res://scripts/main_v0141.gd"

const WORKSPACE_V0142 = preload("res://scripts/ui/workspace_v0142.gd")
const TEXT_INPUT_CONVENTION_SERVICE = preload("res://scripts/ui/text_input_convention_service.gd")
const BUILD_DISPLAY_VERSION_V0142 := "0.14.2"

var _text_input_convention_service: CCFTextInputConventionService


func _ready() -> void:
	super._ready()
	if _text_input_convention_service == null:
		_text_input_convention_service = TEXT_INPUT_CONVENTION_SERVICE.new()
		add_child(_text_input_convention_service)
...
func _install_workspace_v0133() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0142:
		return
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0142.new()
...
func _update_build_version_label() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0142
