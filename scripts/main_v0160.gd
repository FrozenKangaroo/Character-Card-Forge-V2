extends "res://scripts/main_v01540_hotfix9.gd"

const WORKSPACE_V0160 = preload("res://scripts/ui/workspace_v0160.gd")
const BUILD_DISPLAY_VERSION_V0160 := "0.16.0"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0160()
...
func _install_workspace_v01526() -> void:
	...
	if (
		...
		and previous_workspace.get_script() == WORKSPACE_V0160
	):
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0160.new()
...
func _update_build_version_label_v0160() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0160
			node.tooltip_text = "Development build version. v0.16 starts the post-v0.15.40 development cycle; release metadata remains synchronised separately by release.sh."
			return
