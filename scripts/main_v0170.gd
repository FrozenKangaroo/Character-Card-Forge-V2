extends "res://scripts/main_v01610.gd"

const WORKSPACE_V0170 = preload("res://scripts/ui/workspace_v0170.gd")
const IMAGE_WINDOW_V0170 = preload(
	"res://scripts/ui/image_generation_window_v0170.gd"
)
const BUILD_DISPLAY_VERSION_V0170 := "0.17.0"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0170()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0170:
	...
	if previous_workspace != null:
		...
		if previous_workspace.library_requested.is_connected(
			_show_library_from_workspace
		):
			previous_workspace.library_requested.disconnect(
				_show_library_from_workspace
			)
		if previous_workspace.settings_requested.is_connected(
			_show_settings_from_workspace
		):
			previous_workspace.settings_requested.disconnect(
				_show_settings_from_workspace
			)
		if previous_workspace.template_manager_requested.is_connected(
			_show_templates_from_workspace
		):
			previous_workspace.template_manager_requested.disconnect(
				_show_templates_from_workspace
			)
		if previous_workspace.series_manager_requested.is_connected(
			_show_series_from_workspace
		):
			previous_workspace.series_manager_requested.disconnect(
				_show_series_from_workspace
			)
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0170.new()
...
func _install_image_window_v01529() -> void:
	...
	if previous != null and previous.get_script() == IMAGE_WINDOW_V0170:
		previous.call("ensure_collaborator_handoff_surface_v0170")
	...
	if previous != null:
		...
		previous.queue_free()
	var upgraded: CCFImageGenerationWindowV0170 = IMAGE_WINDOW_V0170.new()
...
func _on_image_collaborator_handoff_requested_v0170(
	source: Dictionary,
	options: Dictionary
) -> void:
	var result: Dictionary = {
		"ok": false,
		"error": "The active Workspace does not support Image Studio handoff."
	}
	if (
		_workspace != null
		and _workspace.has_method("open_collaborator_with_image_result_v0170")
	):
		var result_value: Variant = _workspace.call(
			"open_collaborator_with_image_result_v0170", source, options
		)
		if result_value is Dictionary:
			result = result_value as Dictionary
	if (
		_image_generation_window != null
		and _image_generation_window.has_method(
			"report_collaborator_handoff_v0170"
		)
	):
		_image_generation_window.call(
			"report_collaborator_handoff_v0170", result
		)
	_global_status.text = str(
		result.get("message", result.get("error", "Image handoff finished."))
	)


func _update_build_version_label_v0170() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0170
			node.tooltip_text = (
				"Development build version. v0.17.0 adds structured Image Studio result handoff to Character Collaborator with optional separate Vision evidence."
			)
			return
