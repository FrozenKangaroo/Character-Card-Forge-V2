...
const CURRENT_BUILD_VERSION := "0.20.9"
...
func _update_current_build_label() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			...
			node.tooltip_text = (
				"v0.20.9 fixes Front Porch work schedules and generated Work Days, "
				+ "adds per-tab field selection, and blocks trailing JSON from contaminating generated greetings."
			)
			return
