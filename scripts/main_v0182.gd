extends "res://scripts/main_v0181.gd"

const WORKSPACE_V0182 = preload("res://scripts/ui/workspace_v0182.gd")
const BUILD_DISPLAY_VERSION_V0182 := "0.18.2"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0182()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0182:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0182.new()
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0182
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0182() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0182
			node.tooltip_text = (
				"v0.18.2 adds advisory card health checks, section token estimates, "
				+ "compiled-prompt approximation, technical metadata and validated Raw "
				+ "JSON, richer import/migration review, missing-asset repair and a "
				+ "private quality checklist."
			)
			return
