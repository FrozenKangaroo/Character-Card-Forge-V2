extends "res://scripts/main_v0170.gd"

const WORKSPACE_V0171 = preload("res://scripts/ui/workspace_v0171.gd")
const BUILD_DISPLAY_VERSION_V0171 := "0.17.1"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0171()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0171:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0171.new()
...
func _update_build_version_label_v0171() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0171
			node.tooltip_text = (
				"Development build version. v0.17.1 adds explicit Collaborator evidence roles and conflict review for structured sources, Image Studio intent and linked Vision observations."
			)
			return
