extends "res://scripts/main_v0164.gd"

const IMAGE_WINDOW_V0165 = preload(
	"res://scripts/ui/image_generation_window_v0165.gd"
)
const BUILD_DISPLAY_VERSION_V0165 := "0.16.5"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0165()


func _install_image_window_v01529() -> void:
	var previous := _image_generation_window
	if previous != null and previous.get_script() == IMAGE_WINDOW_V0165:
	...
	if previous != null:
		...
		previous.queue_free()
	var upgraded := IMAGE_WINDOW_V0165.new()
...
func _update_build_version_label_v0165() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0165
			node.tooltip_text = (
				"Development build version. v0.16.5 adds checkpoint-specific Forge/A1111 profiles, optional family defaults and explicit local capability overrides."
			)
			return
