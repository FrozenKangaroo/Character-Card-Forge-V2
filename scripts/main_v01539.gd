extends "res://scripts/main_v01538.gd"

const WORKSPACE_V01539 = preload(
	"res://scripts/ui/workspace_v01539.gd"
)
const BUILD_DISPLAY_VERSION_V01539 := "0.15.39-hotfix1"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v01539()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V01539:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V01539.new()
...
func _update_build_version_label_v01539() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V01539
			node.tooltip_text = (
