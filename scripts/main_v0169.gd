extends "res://scripts/main_v0168.gd"

const IMAGE_WINDOW_V0169 = preload(
	"res://scripts/ui/image_generation_window_v0169.gd"
)
const BUILD_DISPLAY_VERSION_V0169 := "0.16.9"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0169()


func _install_image_window_v01529() -> void:
	var previous := _image_generation_window
	if previous != null and previous.get_script() == IMAGE_WINDOW_V0169:
		...
		if previous.has_method("ensure_style_preset_surface_v0169"):
			previous.ensure_style_preset_surface_v0169()
	...
	if previous != null:
		...
		previous.queue_free()
	var upgraded: CCFImageGenerationWindowV0169 = IMAGE_WINDOW_V0169.new()
...
func _update_build_version_label_v0169() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0169
			node.tooltip_text = (
				"Development build version. v0.16.9 adds portable Image Style Presets with reusable Global presets, project visual identity and per-character defaults."
			)
			return
