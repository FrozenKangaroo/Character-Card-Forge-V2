extends "res://scripts/main_v01521.gd"

const WORKSPACE_V01522 = preload("res://scripts/ui/workspace_v01522.gd")
const SETTINGS_VIEW_V01522 = preload("res://scripts/ui/settings_view_v01522.gd")
const BUILD_DISPLAY_VERSION_V01522 := "0.15.22"


func _ready() -> void:
	super._ready()
	_install_settings_view_v01522()
...
func _install_workspace_v0133() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V01522:
		return
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V01522.new()
...
func _install_settings_view_v01522() -> void:
	if _content == null:
		return
	var previous_settings: CCFSettingsView = _settings_view
	if previous_settings != null and previous_settings.get_script() == SETTINGS_VIEW_V01522:
		return
	...
	var upgraded: CCFSettingsView = SETTINGS_VIEW_V01522.new()
	upgraded.visible = should_be_visible
...
func _update_build_version_label() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V01522
