extends "res://scripts/main_v0175.gd"

const WORKSPACE_V0180 = preload("res://scripts/ui/workspace_v0180.gd")
const BUILD_DISPLAY_VERSION_V0180 := "0.18.0"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0180()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0180:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0180.new()
...
func _update_build_version_label_v0180() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0180
			node.tooltip_text = (
				"Development build version. v0.18.0 adds lossless Front Porch .fpworld "
				+ "authoring and exchange, optional climate/lore editing, Stoop-ready "
				+ "metadata and an explicit privacy/adult-content export review."
			)
			return
