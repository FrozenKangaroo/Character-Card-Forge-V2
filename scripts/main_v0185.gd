extends "res://scripts/main_v0184.gd"

const WORKSPACE_V0185 = preload("res://scripts/ui/workspace_v0185.gd")
const LIBRARY_V0185 = preload("res://scripts/ui/library_view_v0185.gd")
const BUILD_DISPLAY_VERSION_V0185 := "0.18.5"


func _ready() -> void:
	super._ready()
	_install_library_v0185()
	_update_build_version_label_v0185()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0185:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0185.new()
...
func _install_library_v0185() -> void:
	...
	if previous_library != null and previous_library.get_script() == LIBRARY_V0185:
		return
	...
	var upgraded := LIBRARY_V0185.new() as CCFLibraryV0185View
	upgraded.visible = should_be_visible
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0185
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0185() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0185
			node.tooltip_text = (
				"v0.18.5 adds credential-safe Front Porch diagnostics, authenticated "
				+ "portrait access, explicit install identity and fingerprints, previewed "
				+ "two-way changes, sequential deployment reports and Library remote-state filters."
			)
			return
