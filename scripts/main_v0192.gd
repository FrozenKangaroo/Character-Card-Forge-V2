extends "res://scripts/main_v0191.gd"

const WORKSPACE_V0192 = preload("res://scripts/ui/workspace_v0192.gd")
const BUILD_DISPLAY_VERSION_V0192 := "0.19.2"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0192()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0192:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0192.new()
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0192
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0192() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0192
			node.tooltip_text = (
				"v0.19.2 adds explicit Front Porch-backed single-character Test Chat, "
				+ "local comparison profiles, live streaming/cancellation and safe "
				+ ".fpchat/SillyTavern exchange with managed recovery copies."
			)
			return
