extends "res://scripts/main_v0180_hotfix3.gd"

const WORKSPACE_V0181 = preload("res://scripts/ui/workspace_v0181.gd")
const BUILD_DISPLAY_VERSION_V0181 := "0.18.1"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0181()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0181:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0181.new()
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0181
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0181() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0181
			node.tooltip_text = (
				"v0.18.1 adds durable character checkpoints, field-aware comparison, "
				+ "non-destructive restore, selective merge, revision forks, earlier-"
				+ "revision export and retention/package controls."
			)
			return
