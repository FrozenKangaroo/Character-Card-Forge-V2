extends "res://scripts/main_v0185.gd"

const WORKSPACE_V0186 = preload("res://scripts/ui/workspace_v0186.gd")
const BUILD_DISPLAY_VERSION_V0186 := "0.18.6"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0186()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0186:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0186.new()
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0186
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0186() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0186
			node.tooltip_text = (
				"v0.18.6 creates independent Compact/Lite derivatives from finished "
				+ "characters with target budgets, four compression levels, preservation "
				+ "controls, complete editable comparison, private lineage and revision safety."
			)
			return
