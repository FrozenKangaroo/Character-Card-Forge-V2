extends "res://scripts/main_v0173_hotfix1.gd"

const WORKSPACE_V0174 = preload("res://scripts/ui/workspace_v0174.gd")
const BUILD_DISPLAY_VERSION_V0174 := "0.17.4"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0174()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0174:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0174.new()
...
func _update_build_version_label_v0174() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0174
			node.tooltip_text = (
				"Development build version. v0.17.4 adds portable Front Porch multi-character "
				+ "group-card authoring, AI-or-manual group fields, validation, PNG import/export, "
				+ "member artwork preservation and stable-ID remapping."
			)
			return
