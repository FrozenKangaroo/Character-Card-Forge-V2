extends "res://scripts/main_v01422.gd"

const WORKSPACE_V015 = preload("res://scripts/ui/workspace_v015.gd")
const BUILD_DISPLAY_VERSION_V015 := "0.15.0"
...
func _install_workspace_v0133() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V015:
		return
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V015.new()
...
func _update_build_version_label() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V015
