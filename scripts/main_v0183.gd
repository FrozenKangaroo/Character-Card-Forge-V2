extends "res://scripts/main_v0182.gd"

const WORKSPACE_V0183 = preload("res://scripts/ui/workspace_v0183.gd")
const BUILD_DISPLAY_VERSION_V0183 := "0.18.3"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0183()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0183:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0183.new()
...
func _update_comparison_version_v0180_hotfix2() -> String:
	if OS.has_feature("editor"):
		return BUILD_DISPLAY_VERSION_V0183
	return super._update_comparison_version_v0180_hotfix2()


func _update_build_version_label_v0183() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0183
			node.tooltip_text = (
				"v0.18.3 adds optional AI consistency review, a visible versioned rubric, "
				+ "content-hash staleness, private review/dismissal history, field-by-field "
				+ "approve/reject/edit controls and revision-safe selective application."
			)
			return
