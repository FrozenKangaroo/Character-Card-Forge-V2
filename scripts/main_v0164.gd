extends "res://scripts/main_v0163.gd"

const IMAGE_WINDOW_V0164 = preload(
	"res://scripts/ui/image_generation_window_v0164.gd"
)
const BUILD_DISPLAY_VERSION_V0164 := "0.16.4"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0164()


func _install_image_window_v01529() -> void:
	var previous := _image_generation_window
	if previous != null and previous.get_script() == IMAGE_WINDOW_V0164:
	...
	if previous != null:
		...
		previous.queue_free()
	var upgraded := IMAGE_WINDOW_V0164.new()
...
func _update_build_version_label_v0164() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0164
			node.tooltip_text = (
				"Development build version. v0.16.4 adds provider-discovered per-model Image capabilities and dynamic technical controls on top of the v0.16.3 tabbed Studio."
			)
			return
