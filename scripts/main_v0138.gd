...
const BUILD_DISPLAY_VERSION_V0138 := "0.13.8"

func _install_workspace_v0133() -> void:
	if _content == null or _workspace is CCFWorkspaceV0138View:
		return
	...
	var upgraded := CCFWorkspaceV0138View.new()
...
func _update_build_version_label() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0138
