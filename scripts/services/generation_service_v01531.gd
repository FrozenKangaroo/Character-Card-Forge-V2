...
func job_records_v01531() -> Array:
	...
	if not _active_job.is_empty():
		var active_id := str(_active_job.get("id", ""))
		var active_status := "running"
		...
		records.append(
			_job_record_v01531(
				...
				_scheduler_worker_label_v01526,
				"",
				active_id,
				true,
				0
	...
	for queue_index in range(_queue.size()):
		...
		var queued_id := str(queued_job.get("id", ""))
		records.append(
			_job_record_v01531(
				queued_job,
				"queued",
...
func cancel_job_v01531(job_id: String) -> bool:
	...
	for index in range(_queue.size()):
		...
		if str(queued_job.get("id", "")) != clean_id:
			continue
		var cancelled_type := str(queued_job.get("type", ""))
...
func worker_identity_v01531() -> Dictionary:
	return {
		"worker_id": _scheduler_worker_id_v01526,
		"label": _scheduler_worker_label_v01526
	}
...
func _job_record_v01531(
	...
	var role := CCFAIJobSchedulerV01526.ROLE_TEXT
	if str(metadata.get("request_role", "")).to_lower() == "vision" or str(
		job.get("type", "")
	) == "vision_analysis":
...
func _safe_section_records_v01531() -> Array:
	...
	var parent_job_id := str(_active_job.get("id", ""))
	var completed_value: Variant = _parallel_coordinator_v01526.get(
		"completed_indices", {}
	)
	...
	var current_wave_value: Variant = _parallel_coordinator_v01526.get(
		"wave_indices", []
	)
	...
	for running_record_value in running.values():
		...
		var section_index := int(running_record.get("section_index", -1))
		if section_index >= 0:
			running_by_index[section_index] = running_record
	...
	for section_index in range(plan.size()):
		...
		if completed.has(section_index):
			...
			if child_record_value is Dictionary:
				...
				if child_service_value is CCFGenerationServiceV01526:
					var child_service := child_service_value as CCFGenerationServiceV01526
					if bool(child_service.get("_scheduler_waiting_v01526")):
						status = "waiting_capacity"
		elif section_index in current_wave:
			status = "queued"
		else:
			var dependency_names: Array[String] = []
			for dependency_index in _section_dependency_indices_v01526(
				plan, section_index
			):
			...
			if not dependency_names.is_empty():
				detail = "Waiting for %s" % ", ".join(dependency_names)
			else:
				detail = "Waiting for an earlier Safe Section wave"
		records.append(
			{
				"record_id": "%s::safe::%03d" % [
					_scheduler_worker_id_v01526, section_index
				],
				"job_id": "%s::%s" % [parent_job_id, section_id],
				"job_type": "safe_section",
				"label": section_title,
				"worker_id": _scheduler_worker_id_v01526,
				"worker_label": _scheduler_worker_label_v01526,
				...
				"generation_strategy": GENERATION_STRATEGY_SAFE_SECTION,
				"stage": "safe_section",
				"detail": detail
			}
		)
...
func _record_stage_v01531(job: Dictionary) -> String:
	var interview_stage := str(job.get("interview_stage", "")).strip_edges()
	...
	var safe_stage := str(job.get("safe_stage", "")).strip_edges()
