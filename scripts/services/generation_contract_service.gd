	...
	if field_rules is Dictionary:
		for raw_field_id in field_rules:
			...
			if required_labels_value is Array and not required_labels_value.is_empty():
				...
				lines.append(
					"- %s must contain these clearly labelled components: %s."
					% [field_id, _join_strings(labels, "; ")]
				)
			var minimum_characters := int(rule.get("minimum_characters", 0))
			if minimum_characters > 0:
				lines.append(
					"- %s should contain at least %d characters of useful content."
					% [field_id, minimum_characters]
				)
			...
			if marker_rules_value is Array:
				for raw_marker_rule in marker_rules_value:
					if not raw_marker_rule is Dictionary:
						continue
					var marker_rule: Dictionary = raw_marker_rule
					var marker := str(marker_rule.get("marker", ""))
					var exact_count := int(marker_rule.get("exact_count", -1))
					if not marker.is_empty() and exact_count >= 0:
						lines.append(
							"- %s must contain marker %s exactly %d time(s) when that optional field is returned."
							% [field_id, marker, exact_count]
						)
...
static func validate_generated_data(data: Dictionary, contract: Dictionary) -> Dictionary:
				...
				issues.append(
					{
						"kind": "missing_field",
						"field_id": field_id,
						"label": str(raw_field.get("label", field_id)),
						"reason": "Required top-level field is missing or empty."
					}
				)
				...
				issues.append(
					{
						...
						"field_id": field_id,
						"label": field_id,
						"reason": "Field is too short for its generation contract (%d/%d characters)."
							% [value_text.strip_edges().length(), minimum_characters]
					}
				)
					...
					issues.append(
						{
							...
							"field_id": field_id,
							"label": label,
							"reason": "Expected labelled component is missing."
						}
					)
					...
					issues.append(
						{
							...
							"field_id": field_id,
							"label": marker,
							"reason": "Expected marker %s exactly %d time(s), found %d."
								% [marker, exact_count, actual_count]
						}
					)
	...
	return {
		...
		"issues": issues,
		"required_field_count": required_lookup.size(),
		"summary": "Generation contract satisfied." if issues.is_empty() else "%d generation-contract issue(s) detected." % issues.size()
	}
