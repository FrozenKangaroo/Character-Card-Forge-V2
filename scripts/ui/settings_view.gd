...
func _ready() -> void:
    ...
    var intro := Label.new()
    intro.text = "OpenAI-compatible text backends"
    intro.add_theme_font_size_override("font_size", 22)
    add_child(intro)

    var hint := Label.new()
    hint.text = "Create multiple backend profiles, fetch available models from /models, and choose which profile Character Card Forge uses for generation jobs."
    ...
    form.add_child(_label("Text model"))
...
func _on_profile_selected(index: int) -> void:
    ...
    _load_active_profile()
    _status.text = "Active profile changed locally. Press Save Settings to persist it."
...
func _create_profile() -> void:
    ...
    _refresh_profile_selector()
    _load_active_profile()
    _status.text = "New API profile created."
...
func _duplicate_profile() -> void:
    ...
    _refresh_profile_selector()
    _load_active_profile()
    _status.text = "API profile duplicated."
...
func _delete_profile() -> void:
    ...
    _refresh_profile_selector()
    _load_active_profile()
    _status.text = "API profile deleted."
