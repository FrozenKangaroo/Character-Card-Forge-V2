...
func set_jobs_v01531(records: Array, scheduler_snapshot: Dictionary) -> void:
	...
	for child in _list.get_children():
		child.queue_free()
	var running := int(scheduler_snapshot.get("running", 0))
	var waiting := int(scheduler_snapshot.get("waiting", 0))
	var counted := int(scheduler_snapshot.get("running_counted", 0))
...
func _build_job_row_v01531(record: Dictionary) -> Control:
	...
	var status := str(record.get("status", "queued"))
	...
	var worker_label := str(record.get("worker_label", "")).strip_edges()
	if not worker_label.is_empty() and parent_id.is_empty():
		detail_parts.append(worker_label)
	var role := str(record.get("role", "")).strip_edges()
	if not role.is_empty():
		detail_parts.append(role.capitalize())
	var stage := str(record.get("stage", "")).strip_edges()
	if not stage.is_empty():
		detail_parts.append(stage)
	var profile_name := str(record.get("profile_name", "")).strip_edges()
	if not profile_name.is_empty():
		detail_parts.append(profile_name)
	var model := str(record.get("model", "")).strip_edges()
	if not model.is_empty():
		detail_parts.append(model)
	var detail := str(record.get("detail", "")).strip_edges()
	if not detail.is_empty():
		detail_parts.append(detail)
	var queue_position := int(record.get("queue_position", 0))
	...
	if bool(record.get("can_cancel", false)):
		...
		var worker_id := str(record.get("worker_id", ""))
		var cancel_job_id := str(record.get("cancel_job_id", record.get("job_id", "")))
