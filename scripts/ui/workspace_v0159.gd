class_name CCFWorkspaceV0159View
extends "res://scripts/ui/workspace_v0158.gd"

const GENERATION_SERVICE_V0159 = preload("res://scripts/services/generation_service_v0159.gd")


func _install_generation_service_v015() -> void:
	var previous_service: CCFGenerationService = _generation_service
	if previous_service != null and previous_service.get_script() == GENERATION_SERVICE_V0159:
		return
	...
	var upgraded: CCFGenerationService = GENERATION_SERVICE_V0159.new()
	add_child(upgraded)
...
func _ensure_collaborator_generation_service_v015() -> bool:
	if _generation_service == null or _generation_service.get_script() != GENERATION_SERVICE_V0159:
		_install_generation_service_v015()
	if _generation_service == null or _generation_service.get_script() != GENERATION_SERVICE_V0159:
		_status.text = "Character Collaborator could not activate the v0.15.9 Vision-limits generation service."
		return false
	if _character_collaborator_window != null:
		_character_collaborator_window.set_generation_service(_generation_service)
	return true
