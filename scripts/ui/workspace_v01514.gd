class_name CCFWorkspaceV01514View
extends "res://scripts/ui/workspace_v01513.gd"

const GENERATION_SERVICE_V01514 = preload("res://scripts/services/generation_service_v01514.gd")


func _install_generation_service_v015() -> void:
	var previous_service: CCFGenerationService = _generation_service
	if previous_service != null and previous_service.get_script() == GENERATION_SERVICE_V01514:
		return
	...
	var upgraded: CCFGenerationService = GENERATION_SERVICE_V01514.new()
	add_child(upgraded)
...
func _ensure_collaborator_generation_service_v015() -> bool:
	if _generation_service == null or _generation_service.get_script() != GENERATION_SERVICE_V01514:
		_install_generation_service_v015()
	if _generation_service == null or _generation_service.get_script() != GENERATION_SERVICE_V01514:
		_status.text = "Character Collaborator could not activate the v0.15.14 generation service."
		return false
	if _character_collaborator_window != null:
		_character_collaborator_window.set_generation_service(_generation_service)
	return true
