...
func _ready() -> void:
	super._ready()
	_update_build_version_label_v01538()


func _install_image_window_v01529() -> void:
	var previous := _image_generation_window
	if previous != null and previous.get_script() == IMAGE_WINDOW_V01538:
	...
	if previous != null:
		...
		previous.queue_free()
	var upgraded := IMAGE_WINDOW_V01538.new() as CCFImageGenerationWindowV01538
...
func _update_build_version_label_v01538() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V01538
			node.tooltip_text = (
