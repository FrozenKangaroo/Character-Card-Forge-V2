extends "res://scripts/main_v0166.gd"

const WORKSPACE_V0167 = preload("res://scripts/ui/workspace_v0167.gd")
const BUILD_DISPLAY_VERSION_V0167 := "0.16.7"


func _ready() -> void:
	super._ready()
	_update_build_version_label_v0167()
...
func _install_workspace_v01526() -> void:
	...
	if previous_workspace != null and previous_workspace.get_script() == WORKSPACE_V0167:
	...
	var upgraded: CCFWorkspaceView = WORKSPACE_V0167.new()
...
func _update_build_version_label_v0167() -> void:
	for node in find_children("*", "Label", true, false):
		if node is Label and node.text.begins_with("Godot rewrite • v"):
			node.text = "Godot rewrite • v%s" % BUILD_DISPLAY_VERSION_V0167
			node.tooltip_text = (
				"Development build version. v0.16.7 adds data-driven Quick, Standard, Detailed and Extended Idea Generator detail levels with prompt and output-budget control."
			)
			return
