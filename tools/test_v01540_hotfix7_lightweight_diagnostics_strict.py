...
SUCCESS_MARKER = "v0.15.40-hotfix7 lightweight diagnostics regression passed"
ERROR_PATTERNS = (
    re.compile(r"SCRIPT ERROR:", re.IGNORECASE),
    re.compile(r"Assertion failed", re.IGNORECASE),
    re.compile(r"V01540_HOTFIX7_DIAGNOSTICS_ERROR", re.IGNORECASE),
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01540_hotfix7_lightweight_diagnostics.gd",
    ]
    with tempfile.TemporaryDirectory(prefix="ccf-v01540-hotfix7-diagnostics-") as tmp:
        try:
            ...
            print(
                "ERROR: v0.15.40-hotfix7 diagnostics regression timed out; opening Diagnostics may have blocked the Godot UI thread.",
                file=sys.stderr,
            )
            return 124
    ...
    if matched:
        print(
            "ERROR: forbidden regression output: " + ", ".join(matched),
            file=sys.stderr,
        )
        return 1
