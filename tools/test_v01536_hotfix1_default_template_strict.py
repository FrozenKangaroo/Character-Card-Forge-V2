#!/usr/bin/env python3
"""Run the v0.15.36-hotfix1 configured-default-template regression strictly."""
...
SUCCESS_MARKER = "v0.15.36-hotfix1 default template regression passed"
ERROR_PATTERNS = (
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01536_hotfix1_default_template.gd",
    ]
    with tempfile.TemporaryDirectory(prefix="ccf-v01536-hotfix1-") as tmp:
        try:
            ...
            print("ERROR: v0.15.36-hotfix1 default template regression timed out.", file=sys.stderr)
            return 124
    ...
    if completed.returncode != 0:
        print(f"ERROR: hotfix regression exited with {completed.returncode}.", file=sys.stderr)
        return completed.returncode
    matched = [pattern.pattern for pattern in ERROR_PATTERNS if pattern.search(output)]
    if matched:
        print("ERROR: hotfix regression logged a forbidden error: " + ", ".join(matched), file=sys.stderr)
        return 1
    if SUCCESS_MARKER not in output:
        print("ERROR: hotfix regression did not print its success marker.", file=sys.stderr)
        return 1
    return 0
