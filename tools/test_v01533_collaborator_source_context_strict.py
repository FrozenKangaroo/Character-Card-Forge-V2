#!/usr/bin/env python3
"""Run the v0.15.33 Collaborator source-context regression strictly."""
...
SUCCESS_MARKER = "v0.15.33 Collaborator source context regression passed"
ERROR_PATTERNS = (
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01533_collaborator_source_context.gd",
    ]
    try:
        ...
        print("ERROR: v0.15.33 Collaborator source-context regression timed out.", file=sys.stderr)
        return 124
    ...
    if completed.returncode != 0:
        print(f"ERROR: v0.15.33 regression exited with {completed.returncode}.", file=sys.stderr)
        return completed.returncode
    matched = [pattern.pattern for pattern in ERROR_PATTERNS if pattern.search(output)]
    if matched:
        print(
            "ERROR: v0.15.33 regression logged a forbidden error: " + ", ".join(matched),
            file=sys.stderr,
        )
        return 1
    if SUCCESS_MARKER not in output:
        print("ERROR: v0.15.33 regression did not print its success marker.", file=sys.stderr)
        return 1
    return 0
