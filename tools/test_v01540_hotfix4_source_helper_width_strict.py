...
SUCCESS_MARKER = "v0.15.40-hotfix4 source helper width regression passed"
ERROR_PATTERNS = (
    re.compile(r"SCRIPT ERROR:", re.IGNORECASE),
    re.compile(r"Assertion failed", re.IGNORECASE),
    re.compile(r"V01540_HOTFIX4_SOURCE_HELPER_ERROR", re.IGNORECASE),
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01540_hotfix4_source_helper_width.gd",
    ]
    with tempfile.TemporaryDirectory(prefix="ccf-v01540-hotfix4-source-helper-") as tmp:
        try:
            ...
            print("ERROR: v0.15.40-hotfix4 source-helper regression timed out.", file=sys.stderr)
            return 124
