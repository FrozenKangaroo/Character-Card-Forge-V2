...
func _init() -> void:
	...
	assert(workspace_source.contains("_ccfchar_preview = Window.new()"), "Import preview must use a normal Window so its action bar is explicit and stable.")
	assert(workspace_source.count("force_native = true") >= 2, "CCFCHAR file picker and preview must be detachable native windows.")
