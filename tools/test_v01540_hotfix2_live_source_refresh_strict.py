...
SUCCESS_MARKER = "v0.15.40-hotfix2 live Collaborator source refresh regression passed"
ERROR_PATTERNS = (
    re.compile(r"SCRIPT ERROR:", re.IGNORECASE),
    re.compile(r"Assertion failed", re.IGNORECASE),
    re.compile(r"V01540_HOTFIX2_LIVE_SOURCE_REFRESH_ERROR", re.IGNORECASE),
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01540_hotfix2_live_source_refresh.gd",
    ]
    with tempfile.TemporaryDirectory(prefix="ccf-v01540-hotfix2-live-source-refresh-") as tmp:
        try:
            ...
            print("ERROR: v0.15.40-hotfix2 live-source regression timed out.", file=sys.stderr)
            return 124
