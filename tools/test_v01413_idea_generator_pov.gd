...
func _init() -> void:
	...
	var newer_main := FileAccess.get_file_as_string("res://scripts/main_v01414.gd")
	assert(newer_main.contains("extends \"res://scripts/main_v01413.gd\""), "Newer shells must inherit the v0.14.13 Idea Generator POV layer.")
	var scene := FileAccess.get_file_as_string("res://scenes/main.tscn")
	assert(scene.contains("main_v01414.gd"), "Main scene must advance through a shell that retains v0.14.13 behaviour.")
