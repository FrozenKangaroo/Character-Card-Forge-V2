...
SUCCESS_MARKER = "v0.15.40-hotfix1 Collaborator source row layout regression passed"
ERROR_PATTERNS = (
    re.compile(r"SCRIPT ERROR:", re.IGNORECASE),
    re.compile(r"Assertion failed", re.IGNORECASE),
    re.compile(r"V01540_HOTFIX1_SOURCE_ROW_REGRESSION_ERROR", re.IGNORECASE),
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01540_hotfix1_collaborator_source_rows.gd",
    ]
    with tempfile.TemporaryDirectory(prefix="ccf-v01540-hotfix1-source-rows-") as tmp:
        try:
            ...
            print("ERROR: v0.15.40-hotfix1 source-row regression timed out.", file=sys.stderr)
            return 124
