...
func _run() -> void:
	...
	if not _require(
		...
		"Adding an existing library character must create an independent, revisioned copy without broken managed-file links."
	):
		return
	...
	for node in app.find_children("*", "Label", true, false):
		if node is Label and node.text == "Godot rewrite • v0.19.0":
			version_found = true
			break
	if not _require(
