...
SUCCESS_MARKER = "v0.15.40-hotfix3 whole Collaborator sidebar final-reflow regression passed"
ERROR_PATTERNS = (
    re.compile(r"SCRIPT ERROR:", re.IGNORECASE),
    re.compile(r"Assertion failed", re.IGNORECASE),
    re.compile(r"V01540_HOTFIX3_SIDEBAR_REFLOW_ERROR", re.IGNORECASE),
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01540_hotfix3_sidebar_final_reflow.gd",
    ]
    with tempfile.TemporaryDirectory(prefix="ccf-v01540-hotfix3-sidebar-final-reflow-") as tmp:
        try:
            ...
            print("ERROR: v0.15.40-hotfix3 whole-sidebar regression timed out.", file=sys.stderr)
            return 124
