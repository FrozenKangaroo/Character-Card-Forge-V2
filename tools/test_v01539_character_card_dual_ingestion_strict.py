...
SUCCESS_MARKER = "v0.15.39 Character Card PNG dual-ingestion regression passed"
ERROR_PATTERNS = (
    re.compile(r"SCRIPT ERROR:", re.IGNORECASE),
    re.compile(r"Assertion failed", re.IGNORECASE),
    re.compile(r"V01539_REGRESSION_ERROR", re.IGNORECASE),
...
def main() -> int:
    ...
    command = [
        ...
        str(REPO_ROOT),
        "--script",
        "res://tools/test_v01539_character_card_dual_ingestion.gd",
    ]
    with tempfile.TemporaryDirectory(prefix="ccf-v01539-dual-card-") as tmp:
        try:
            ...
            print("ERROR: v0.15.39 dual-ingestion regression timed out.", file=sys.stderr)
            return 124
