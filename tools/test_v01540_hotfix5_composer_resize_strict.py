...
SUCCESS_MARKER = "v0.15.40-hotfix5 composer resize and warning regression passed"
ERROR_PATTERNS = (
    re.compile(r"SCRIPT ERROR:", re.IGNORECASE),
    re.compile(r"Assertion failed", re.IGNORECASE),
    re.compile(r"V01540_HOTFIX5_COMPOSER_ERROR", re.IGNORECASE),
...
def main() -> int:
    try:
        ...
        return 2
    command = [godot, "--headless", "--path", str(REPO_ROOT), "--script", "res://tools/test_v01540_hotfix5_composer_resize.gd"]
    with tempfile.TemporaryDirectory(prefix="ccf-v01540-hotfix5-composer-") as tmp:
        try:
            ...
            print("ERROR: v0.15.40-hotfix5 composer regression timed out.", file=sys.stderr)
            return 124
