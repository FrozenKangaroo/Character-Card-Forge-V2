...
- Replaced the four active v0.20.2–v0.20.5 application-shell layers with one semantic current shell extending the stable v0.20.1 boundary.
- Preserved the Help Center, Support & Diagnostics, permanent navigation, Quick Actions and update-comparison behavior without deleting historical implementation files.
...
- Added one reusable source/package preflight with semantic version, optional tag and complete version-matched changelog validation.
- Added a canonical changelog contract with explicit Highlights, Changes, Migration notes, Breaking changes and Known limitations sections.
- Replaced GitHub-generated release notes with the reviewed matching changelog section.
- Verified the exact Windows, Linux and unsigned macOS package set, non-empty payloads and every recorded SHA-256 checksum before publication.
...
- Required meaningful AI-generated Lorebook entry names, preserved exact planned names from Generation Concepts and added deterministic fallbacks for unnamed existing entries without promoting content warnings into titles.
- Added focused release-contract, tampered-package, live Expression Set window and Lorebook naming coverage to the complete 151-test inherited release profile.
